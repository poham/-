...
interface RimLightPanelProps {
  ...
  keyAzimuth: number; // 用於顯示背後位置
  disabled?: boolean;
  onChange: (rimLight: { elevation: number; color: string; intensity: number; styleDescription?: string }) => void;
}

/**
 * RimLightPanel - Rim Light 的簡化控制面板
 * 只控制仰角（elevation），方位角自動鎖定在主光源背後
 */
const RimLightPanel: React.FC<RimLightPanelProps> = ({
  rimLight,
  keyAzimuth,
  disabled = false,
  onChange
  ...
  const handleElevationChange = (elevation: number) => {
    onChange({ ...rimLight, elevation });
  };
  
  const handleColorChange = (color: string) => {
    onChange({ ...rimLight, color });
  };
  
  const handleIntensityChange = (intensity: number) => {
    onChange({ ...rimLight, intensity });
  };
  
  const handleStyleDescriptionChange = (styleDescription: string) => {
    onChange({ ...rimLight, styleDescription });
  };
  
  const [showPresets, setShowPresets] = useState(false);
  
  // 計算 Rim Light 的方位角（總是在背後）
  ...
  return (
    <div className={`space-y-8 animate-in fade-in duration-500 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <input 
            ...
            min="0" 
            max="100" 
            value={rimLight.intensity} 
            onChange={(e) => handleIntensityChange(parseInt(e.target.value))}
            disabled={disabled}
            className="w-full h-2.5 bg-slate-800 rounded-lg appearance-none accent-yellow-500 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          />
        </div>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-800">
        ...
        {/* 可編輯的 Textarea */}
        <textarea
          value={rimLight.styleDescription || ''}
          onChange={(e) => handleStyleDescriptionChange(e.target.value)}
          disabled={disabled}
          placeholder="描述這個光源的表現特質（如：強烈飽和、柔和擴散等）"
          className="w-full h-24 p-4 bg-slate-900 border-2 border-slate-800 rounded-lg text-[13px] text-slate-300 placeholder-slate-600 focus:border-yellow-500 focus:outline-none resize-none disabled:opacity-50 disabled:cursor-not-allowed"
        />
